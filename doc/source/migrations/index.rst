...
- **Tag Repository Version:** The ``hk-tag-repository-version`` action creates and updates ``vX`` and ``vX.Y``
  ...
  release event. See :doc:`../housekeeping-actions/index` for usage details.
...
- **Safety Ignore File:** The ``check-vulnerabilities`` action now supports custom safety policy files, which
...
- **Check-PR-Title Token Optional:** The ``check-pr-title`` action no longer requires a ``token`` input.
